...
Displays allocation pie chart, sector breakdown bar chart,
and holding performance comparison table.
"""
...
def render_analytics() -> None:
    ...
    # Holding Performance Comparison
    ...
    if performance:
        for p in performance:
            ...
            col3.metric(
                "Gain/Loss",
                f"${p.absolute_gain:+,.2f}",
                delta_color="normal" if p.absolute_gain >= 0 else "inverse",
            )
            col4.metric(
                "Return",
                f"{p.percentage_gain:+.2f}%",
                delta_color="normal" if p.percentage_gain >= 0 else "inverse",
            )
    else:
    ...
    if len(holdings) == 1:
        st.warning(
            "⚠️ Your portfolio consists of a single holding (100% allocation). "
            "Consider diversifying across multiple stocks and sectors to reduce risk."
        )
        ...
        if max_allocation > 50.0:
            ...
            st.warning(
                f"⚠️ {dominant_ticker} represents {max_allocation:.1f}% of "
                "your portfolio. Consider rebalancing to reduce concentration risk."
            )
        else:
