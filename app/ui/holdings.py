...
def render_holdings() -> None:
    ...
    with st.form("add_holding_form", clear_on_submit=True):
        ...
        with col1:
            new_ticker = st.text_input("Ticker Symbol", placeholder="e.g., AAPL", key="new_ticker")
        with col2:
        ...
        if submitted:
            if not new_ticker.strip():
                ...
                error_message(title="Invalid quantity", message="Quantity must be greater than zero.")
            elif new_price <= 0:
                error_message(title="Invalid price", message="Purchase price must be greater than zero.")
            else:
                existing = holding_repo.get_by_ticker(new_ticker)
                if existing:
                    error_message(title="Duplicate holding", message=f"You already hold {existing.ticker}. Edit the existing holding instead.")
                else:
                    with st.spinner("Validating ticker symbol..."):
                        is_valid = market_data_service.validate_ticker(new_ticker.strip())
                    if not is_valid:
    ...
    if not holdings:
        empty_state(title="No holdings yet", message="Use the form above to add your first investment holding.")
        session.close()
        return
        ...
        with st.container(border=True):
            ...
            if st.session_state.get(f"editing_{holding.id}"):
                with st.form(f"edit_form_{holding.id}"):
                    edit_qty = st.number_input("Quantity", value=float(holding.quantity), min_value=0.0, step=0.01, key=f"edit_qty_{holding.id}")
